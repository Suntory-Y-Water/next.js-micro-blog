...
const ArticleCard = ({ article }: ArticleCardProps) => {
  return (
    <article className='shadow flex flex-col my-4' key={article.id}>
      <Link href={`javascript/articles/${article.id}`} className='hover:opacity-75'>
        <Image
      ...
      <div className='bg-white flex flex-col p-6'>
        <Link
          href={`javascript/articles/${article.id}`}
        ...
        <Link href={`javascript/articles/${article.id}`} className='text-slate-700 pb-6'>
